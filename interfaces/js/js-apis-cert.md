...
| ERR_CERT_UNTRUSTED<sup>26.0.0+</sup>               | 19030009 | 证书不受信任。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。    |
| ERR_CERT_HAS_REVOKED<sup>26.0.0+</sup>             | 19030010 | 证书已被吊销。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。    |
| ERR_UNKNOWN_CRITICAL_EXTENSION<sup>26.0.0+</sup>   | 19030011 | 未知的关键扩展。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_CERT_HOSTNAME_MISMATCH<sup>26.0.0+</sup>       | 19030012 | 证书主机名不匹配。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_CERT_EMAIL_ADDRESS_MISMATCH<sup>26.0.0+</sup>  | 19030013 | 证书邮箱地址不匹配。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_CERT_KEYUSAGE_MISMATCH<sup>26.0.0+</sup>       | 19030014 | 证书密钥用途不匹配。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_CRL_NOT_FOUND<sup>26.0.0+</sup>                | 19030015 | 无法获取证书吊销列表。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_CRL_NOT_YET_VALID<sup>26.0.0+</sup>            | 19030016 | 证书吊销列表尚未生效。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_CRL_HAS_EXPIRED<sup>26.0.0+</sup>              | 19030017 | 证书吊销列表已过期。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_CRL_SIGNATURE_FAILURE<sup>26.0.0+</sup>        | 19030018 | 证书吊销列表签名验证失败。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_CRL_ISSUER_NOT_FOUND<sup>26.0.0+</sup>         | 19030019 | 无法获取证书吊销列表颁发者。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_OCSP_RESPONSE_NOT_FOUND<sup>26.0.0+</sup>      | 19030020 | 无法获取OCSP响应。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_OCSP_RESPONSE_INVALID<sup>26.0.0+</sup>        | 19030021 | OCSP响应无效。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_OCSP_SIGNATURE_FAILURE<sup>26.0.0+</sup>       | 19030022 | OCSP签名验证失败。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_OCSP_CERT_STATUS_UNKNOWN<sup>26.0.0+</sup>     | 19030023 | OCSP证书状态未知。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
| ERR_NETWORK_TIMEOUT<sup>26.0.0+</sup>              | 19030024 | 网络连接超时。 <br> **原子化服务API：** 从API version 26.0.0开始，该接口支持在原子化服务中使用。 |
...
## CertRevocationFlag<sup>26.0.0+</sup>
...
| 名称                                  | 值   | 说明                          |
| --------------------------------------| -------- | -----------------------------|
| CERT_REVOCATION_PREFER_OCSP | 0 | 优先使用OCSP检查证书吊销状态。默认情况下，如果同时设置了CRL_CHECK和OCSP_CHECK，会优先使用CRL；设置此标志后，会优先使用OCSP。 |
| CERT_REVOCATION_CRL_CHECK | 1 | 使用CRL检查证书吊销状态。如果未设置CERT_REVOCATION_CHECK_ALL_CERT，仅检查第一个证书的吊销状态。 |
| CERT_REVOCATION_OCSP_CHECK | 2 | 使用OCSP检查证书吊销状态。如果未设置CERT_REVOCATION_CHECK_ALL_CERT，仅检查第一个证书的吊销状态。 |
| CERT_REVOCATION_CHECK_ALL_CERT | 3 | 检查除自签名证书外所有证书的吊销状态。必须与CERT_REVOCATION_CRL_CHECK或CERT_REVOCATION_OCSP_CHECK配合使用。 |

## OcspDigest<sup>26.0.0+</sup>
...
## X509CertRevokedParams<sup>26.0.0+</sup>
...
| 名称         | 类型                                              | 只读 | 可选 |说明                                   |
| ------------ | ------------------------------------------------- | ---- | ---- |--------------------------------------|
| revocationFlags | Array\<[CertRevocationFlag](#certrevocationflag26.0.0)> | 否   | 否   | 吊销检查标志。 |
| crls | Array\<[X509CRL](#x509crl)> | 否   | 是   | 用于验证证书吊销状态的CRL列表。 |
| allowDownloadCrl | boolean | 否   | 是   | 是否允许从网络下载CRL。默认值为false。如果crls中存在匹配的CRL，则跳过下载。 |
| ocspResponses | Array\<Uint8Array> | 否   | 是   | 用于验证证书的OCSP响应数据。 |
| allowOcspCheckOnline | boolean | 否   | 是   | 是否允许在线OCSP检查。默认值为false。如果在ocspResponses中找到匹配的OCSP响应，则跳过在线检查。 |
| ocspDigest | [OcspDigest](#ocspdigest26.0.0) | 否   | 是   | OCSP请求使用的摘要算法。默认值为SHA256。 |

## X509CertValidatorParams<sup>26.0.0+</sup>
...
| 名称         | 类型                                              | 只读 | 可选 |说明                                   |
| ------------ | ------------------------------------------------- | ---- | ---- |--------------------------------------|
| untrustedCerts | Array\<[X509Cert](#x509cert)> | 否   | 是   | 不信任的中间证书，用于构建证书链。 |
| trustedCerts | Array\<[X509Cert](#x509cert)> | 否   | 是   | 信任的锚证书，用于验证证书链。 |
| trustSystemCa | boolean | 否   | 是   | 是否信任系统CA证书来验证证书链。默认值为false。 |
| partialChain | boolean | 否   | 是   | 是否允许部分证书链验证。默认值为false。 |
| allowDownloadIntermediateCa | boolean | 否   | 是   | 是否允许从网络下载中间CA证书。默认值为false。 |
| date | string | 否   | 是   | 用于检查证书或CRL有效性的日期。格式为YYMMDDHHMMSSZ或YYYYMMDDHHMMSSZ。默认使用当前系统时间。 |
| validateDate | boolean | 否   | 是   | 是否检查证书或CRL的有效日期。默认值为true。 |
| ignoreErrs | Array\<[CertResult](#certresult)> | 否   | 是   | 忽略指定的错误。例如，可以使用CertResult.ERR_CERT_HAS_EXPIRED来忽略证书过期错误。 |
| hostnames | Array\<string> | 否   | 是   | 用于验证证书的主机名列表。 |
| emailAddresses | Array\<string> | 否   | 是   | 用于验证证书的邮箱地址列表。目前仅支持单个邮箱地址。 |
| keyUsage | Array\<[KeyUsageType](#keyusagetype12)> | 否   | 是   | 用于验证证书的密钥用途列表。 |
| userId | Uint8Array | 否   | 是   | SM2用户ID。如果验证SM2证书，可以使用此参数配置SM2用户ID。最常见的SM2用户ID是[0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38]。 |
| revokedParams | [X509CertRevokedParams](#x509certrevokedparams26.0.0) | 否   | 是   | 证书吊销检查参数。 |

## VerifyCertResult<sup>26.0.0+</sup>
...
| 名称         | 类型                                              | 只读 | 可选 |说明                                   |
| ------------ | ------------------------------------------------- | ---- | ---- |--------------------------------------|
| certChain | Array\<[X509Cert](#x509cert)> | 是   | 否   | 验证成功的证书链。 |
...
| params | [X509CertValidatorParams](#x509certvalidatorparams26.0.0) | 是   | 证书验证参数。 |

**返回值：**

| 类型           | 说明        |
| -------------- | ----------- |
| Promise\<[VerifyCertResult](#verifycertresult26.0.0)> | Promise对象，返回验证结果。 |
