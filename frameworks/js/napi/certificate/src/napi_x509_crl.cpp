...
static napi_value GenerateCrlEntryArray(napi_env env, CfArray *array, std::string returnClassName)
{
    ...
    for (uint32_t i = 0; i < array->count; i++) {
        ...
        if (x509CrlEntryClass == nullptr) {
            NAPI_LOG_THROW(env, CF_ERR_MALLOC, "Failed to create a x509CrlEntry class");
            CfObjDestroy(entry);
            entry = nullptr;
...
napi_value NapiX509Crl::GetThisUpdate(napi_env env, napi_callback_info info)
{
    ...
    if (ret != CF_SUCCESS) {
        NAPI_LOG_THROW(env, ret, "get last update failed");
        CfFree(blob);
        blob = nullptr;
        return nullptr;
    }
...
napi_value NapiX509Crl::GetNextUpdate(napi_env env, napi_callback_info info)
{
    ...
    if (ret != CF_SUCCESS) {
        NAPI_LOG_THROW(env, ret, "get next update failed");
        CfFree(blob);
        blob = nullptr;
        return nullptr;
    }
...
static bool GetCrlSerialNumberFromNapiValue(napi_env env, napi_value arg, CfBlob &outBlob)
{
    ...
    if (memcpy_s(serialBuf, sizeof(serialBuf), &tmpData, sizeof(int64_t)) != EOK) {
        NAPI_LOG_THROW(env, CF_ERR_COPY, "copy serialNum failed");
        return false;
    }
...
napi_value NapiX509Crl::GetRevokedCertificate(napi_env env, napi_callback_info info, std::string returnClassName)
{
    ...
    if (ret != CF_SUCCESS) {
        NAPI_LOG_THROW(env, ret, "get revoked cert failed!");
        return nullptr;
    }
    ...
    napi_env env, napi_callback_info info, std::string returnClassName)
{
    ...
    if (ret != CF_SUCCESS) {
        NAPI_LOG_THROW(env, ret, "get revoked cert with cert failed!");
        return nullptr;
    }
...
napi_value NapiX509Crl::GetTBSCertList(napi_env env, napi_callback_info info)
{
    ...
    if (result != CF_SUCCESS) {
        NAPI_LOG_THROW(env, result, "get tbs info failed");
        CfFree(blob);
        blob = nullptr;
        return nullptr;
    }
...
napi_value NapiX509Crl::ToStringEx(napi_env env, napi_callback_info info, CfEncodingType encodingType)
{
    ...
    if (result != CF_SUCCESS) {
        NAPI_LOG_THROW(env, result, "ToStringEx failed");
        return nullptr;
    }
    napi_value returnBlob = nullptr;
    napi_create_string_utf8(env, reinterpret_cast<char *>(blob.data), blob.size, &returnBlob);
...
napi_value NapiX509Crl::GetSignature(napi_env env, napi_callback_info info)
{
    ...
    if (result != CF_SUCCESS) {
        NAPI_LOG_THROW(env, result, "get signature failed");
        CfFree(blob);
        blob = nullptr;
        return nullptr;
    }
...
napi_value NapiX509Crl::GetSigAlgName(napi_env env, napi_callback_info info)
{
    ...
    if (ret != CF_SUCCESS) {
        NAPI_LOG_THROW(env, ret, "get signature alg name failed");
        CfFree(blob);
        blob = nullptr;
        return nullptr;
    }
...
napi_value NapiX509Crl::GetSigAlgOID(napi_env env, napi_callback_info info)
{
    ...
    if (ret != CF_SUCCESS) {
        NAPI_LOG_THROW(env, ret, "get signature alg oid failed");
        CfFree(blob);
        blob = nullptr;
        return nullptr;
    }
...
napi_value NapiX509Crl::GetSigAlgParams(napi_env env, napi_callback_info info)
{
    ...
    if (result != CF_SUCCESS) {
        NAPI_LOG_THROW(env, result, "get signature alg params failed");
        CfFree(blob);
        blob = nullptr;
        return nullptr;
    }
...
napi_value NapiX509Crl::Match(napi_env env, napi_callback_info info)
{
    ...
    if (!BuildX509CrlMatchParams(env, argv[PARAM0], param)) {
        NAPI_LOG_THROW(env, CF_INVALID_PARAMS, "BuildX509CrlMatchParams failed");
...
static napi_value NapiGetIssuerDN(napi_env env, napi_callback_info info)
{
    ...
    napi_unwrap(env, thisVar, reinterpret_cast<void **>(&x509Crl));
    if (x509Crl == nullptr) {
        NAPI_LOG_THROW(env, CF_ERR_NAPI, "x509Crl is nullptr!");
        return nullptr;
    }
...
void NapiX509Crl::CreateX509CrlExecute(napi_env env, void *data)
{
    CfCtx *context = static_cast<CfCtx *>(data);
    CfJsApiId apiId =
        (context->createX509CrlName == "createX509Crl") ? API_CREATE_X509_CRL_DEPRECATED : API_CREATE_X509_CRL;
    HistogramScopeGuard guard(apiId);
...
{
    CfJsApiId apiId = (createName == "createX509Crl") ? API_CREATE_X509_CRL_DEPRECATED : API_CREATE_X509_CRL;
    HistogramScopeGuard guard(apiId);
    size_t argc = ARGS_SIZE_TWO;
    napi_value argv[ARGS_SIZE_TWO] = { nullptr };
    napi_value thisVar = nullptr;
