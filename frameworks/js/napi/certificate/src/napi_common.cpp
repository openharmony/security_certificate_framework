...
static CfResult NapiGetBlobElement(napi_env env, napi_value element, const NapiParamInfo *info, CfBlob &value,
    char **errMsg)
{
...
CfResult NapiGetBlobArrayEx(napi_env env, napi_value arg, const NapiParamInfo *info, CfBlobArray &value, char **errMsg)
{
    NapiArrayBaseInfo arrayInfo = {nullptr, 0};
    CfResult ret = NapiGetArrayBaseInfoEx(env, arg, info, &arrayInfo, errMsg);
    if (ret != CF_SUCCESS) {
    ...
    for (uint32_t i = 0; i < arrayInfo.length; ++i) {
        ...
        ret = NapiGetBlobElement(env, element, info->innerParams, value.data[i], errMsg);
        if (ret != CF_SUCCESS) {
            FreeCfBlobArray(value.data, i);
            value.data = nullptr;
            return ret;
...
CfResult NapiGetInt32ExEx(napi_env env, napi_value arg, const char *name, int32_t &value, char **errMsg)
{
