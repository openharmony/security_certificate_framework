...
static void ValidateX509CertExecute(napi_env env, void *data)
{
    ...
    for (uint32_t i = 0; i < context->certObjCount; i++) {
        ...
        if (ret != CF_SUCCESS) {
            ...
            /* Free already created CfObjects */
            for (uint32_t j = 0; j < i; j++) {
                if (context->certObj[j] != nullptr) {
                    context->certObj[j]->destroy(&context->certObj[j]);
                }
            }
            CfFree(context->certObj);
...
napi_value NapiCertChainValidator::ValidateX509Cert(napi_env env, napi_callback_info info)
{
    ...
    CfResult ret = BuildX509CertValidatorParams(env, argv[PARAM1], context->params);
    if (ret != CF_SUCCESS) {
        LOGE("Build validator params failed!");
        napi_throw(env, CertGenerateBusinessError(env, ret, "Build validator params failed!"));
        CfFree(context);
        return nullptr;
    }
